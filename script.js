...
function filtrar() {
  ...
  if (dataFiltro) {
    filtrado = dados.filter(d => d.data === dataFiltro);
  }
  ...
  filtrado.forEach(item => {
    let parada = Number(item.parada || 0);
    let doca = item.doca;
    let predio = item.predio;

    if (
      (predio === "T&C" && grupoTC.includes(doca)) ||
      (["Body", "Stamp", "PWT"].includes(predio) && grupoOutros.includes(doca))
    ) {
      somaTotal += parada;
  ...
  let html = `<p>⏱️ Total de Paradas: ${somaTotal} min</p>`;

  for (let d in docas) {
    html += `<p>🚪 Doca ${d}: ${docas[d]} min</p>`;
  }
