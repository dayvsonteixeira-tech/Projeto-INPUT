...
function atualizarGraficoGeral() {
    ...
    afterDatasetsDraw(chart){
      ...
      chart.data.datasets.forEach((ds,i)=>{
        ...
        meta.data.forEach((bar,index)=>{
          ...
          ctx.fillText(v+" min",bar.x,bar.y-5);
        });
      });
    }
  };
  ...
  grafico=new Chart(canvas,{
    ...
    data:{
      ...
      datasets:[{label:"Paradas (mês atual)",data:[totalTC,totalOutros,totalGeral]}]
    },
    plugins:[plugin]
  });
}
...
function filtrarData(){
  ...
  filtrado.forEach(i=>{
    ...
    let l=Number(i.recLocal||0);
    let im=Number(i.recImportado||0);
...
function abrirRelatorio(d){
  ...
  let html="<h2>Relatório</h2><table border='1'><tr><th>Data</th><th>Parada</th></tr>";
  r.forEach(x=>{
